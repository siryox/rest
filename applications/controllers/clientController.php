<?php
    class clientController extends controller
    {
        ...
        public function __construct()
        {
            ...
            $this->_client = $this->loadModel('client');
        ...
        public function index()
        {
            ...
            if($this->_security->login($datos))
            {
                ...
                if($nameMethod=='list')
                {
                    $valores = $this->_client->list($parameters);
                }
        ...
        public function create()
        {
            ...
            if($this->_security->login($datos))
            {
                ...
                {
                    if($this->_client->create($parameters))
                    {
        ...
        public function update()
        {
            ...
            if($this->_security->login($datos))
            {
                ...
                {
                    if($valores = $this->_client->update($parameters))
                    {
                        echo json_encode(['date'=>date("F j, Y, g:i a"),'url'=>BASE_URL,'values'=>"Method Success"],true);
                    }else
                        echo json_encode(['date'=>date("F j, Y, g:i a"),'url'=>BASE_URL,'values'=>"Method Faild"],true);
            ...
            public function delete()
            {
                ...
                if($this->_security->login($datos))
                {
                    ...
                    {
                        if($valores = $this->_client->delete($parameters))
                        {
                            echo json_encode(['date'=>date("F j, Y, g:i a"),'url'=>BASE_URL,'values'=>"Method Success"],true);
                        }else
                            echo json_encode(['date'=>date("F j, Y, g:i a"),'url'=>BASE_URL,'values'=>"Method Faild"],true);
